...
class SimCLR(object):
    ...
    def _step(self, model, xis, xjs, n_iter):
        ...
        similarity = torch.cosine_similarity(zis, zjs).mean().item()
        print("Average cosine similarity between embeddings: ", similarity)
    ...
    def train(self):
        ...
        # Inside your train() method before initializing DataParallel
        if self.config['n_gpu'] > 1:
             self.config['gpu_ids'] = '[0]'  # Ensure this matches the available GPU IDs
...
#                                                                last_epoch=-1)
    ...
    def _validate(self, model, valid_loader):
     with torch.no_grad():
        ...
        if counter == 0:
            raise ValueError("Validation loader is empty. Check your dataset and loader settings.")
